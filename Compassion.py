...
def notify():
    ...
    " It's good for your health. So take a break and recharge yourself.", duration=10, icon_path=r'logo.ico')
...
def timer():
    ...
    def increment():
        ...
        if run:
            ...
            # special case when user is active at 24.00 hrs updating the previous day
            if datetime.today().strftime('%H') == '23' and datetime.today().strftime('%M') == '59' and datetime.today().strftime('%S') == '59':
                endofday = threading.Thread(target=updateday, args=())
                endofday.start()
...
def newday():
    ...
    frm = starting_time
    to = datetime.today().strftime('%I') + ':' + datetime.today().strftime('%M') + " " + datetime.today().strftime('%p')
    duration = str(curr_hr - prev_day_hr) + " hour " + str(curr_min - prev_day_min) + " min"
...
def updateday():
    ...
    global starting_time, prev_day_hr, prev_day_min
    ...
    # updaing these variables for special case when the user is active at 24.00 hr (end of day)
    starting_time = (datetime.today().strftime('%I') + ':' + datetime.today().strftime('%M') + " " +
                     datetime.today().strftime('%p'))
    prev_day_min = curr_min
    prev_day_hr = curr_hr
    ...
    prev_hr = 0
    prev_min = 0
    if 48 <= ord(prev_tot[1]) < 58:
        prev_hr = int(prev_tot[0:2])
        if 48 <= ord(prev_tot[9]) < 58:
            prev_min = int(prev_tot[8:10])
        else:
            prev_min = int(prev_tot[8])
    else:
        prev_hr = int(prev_tot[0])
        if 48 <= ord(prev_tot[8]) < 58:
            prev_min = int(prev_tot[7:9])
        else:
            prev_min = int(prev_tot[7])

    dura_hr = 0
    dura_min = 0
    if 48 <= ord(duration[1]) < 58:
        dura_hr = int(duration[0:2])
        if 48 <= ord(duration[9]) < 58:
            dura_min = int(duration[8:10])
        else:
            dura_min = int(duration[8])
    else:
        dura_hr = int(duration[0])
        if 48 <= ord(duration[8]) < 58:
            dura_min = int(duration[7:9])
        else:
            dura_min = int(duration[7])

    upd_hr = prev_hr + dura_hr
    upd_min = prev_min + dura_min
    if upd_min >= 60:
        upd_min -= 60
        upd_hr += 1
...
btn = Button(window, bg='#93e1d8', text="START", cursor='hand2', bd='5', width='7', activebackground='black',
            activeforeground='white', font=('Comic Sans MS', '12'), command=start)
